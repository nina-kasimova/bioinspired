...
fuel_resource_grid = np.full((200, 200), 100)

burn_rate = [2, 2.5, 0, 20, 5, 0]
fire_catchability = [0.15, 0.05, 0, 0.2, 0]
...
def probability(cell, i, j, neighbourstates):
    ...
    for num, neighbour in enumerate(neighbourstates):
        if neighbour == 4:
            fire_prob = fire_catchability[initial_grid[i, j]]
            fire_prob = fire_prob * (1 + wind_probs[num])
            print("wind probs", wind_probs[num])
