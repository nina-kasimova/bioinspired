...
class CAConfig(object):
    ...
    def fill_in_defaults(self):
        ...
        if self.initial_grid is None:
            fillstate = self.states[1] if self.states is not None else 1
            ...
            for i in range(len(self.initial_grid)):
                for j in range (len(self.initial_grid)):
                    #canyone_area = canyon_range[1] * len(self.initial_grid) - canyon_range[0] * len(self.initial_grid)
                    #if i > 0.6 * len(self.initial_grid) and i < canyone_area[0] and j > 0.1 * len(self.initial_grid) and j < canyone_area[1]:
                    # if canyon_range[0][0] * len(self.initial_grid) < i < canyon_range[1][0]:
                    #     if  canyon_range[0][1] * len(self.initial_grid) < j < canyon_range[1][1]:
                    #         self.initial_grid[i, j] = self.states[1]
                    #         print("value:", i,j)
                    #     else:
                    #         self.initial_grid[i, j] = self.states[0]
                    # else:
                    #     self.initial_grid[i, j] = self.states[0]
                    if  canyon_range[0][0] * len(self.initial_grid) < i and i < canyon_range[1][0] * len(self.initial_grid):
                        if  j > canyon_range[0][1] * len(self.initial_grid)  and j < canyon_range[1][1] * len(self.initial_grid):
                            self.initial_grid[j, i] = self.states[1]
                            #print("value:", i,j)
                        else:
                            self.initial_grid[j, i] = self.states[0]
                    else:
                        self.initial_grid[j, i] = self.states[0]
                    # if i < 100:
                    #     self.initial_grid[i,j] = self.states[1]
